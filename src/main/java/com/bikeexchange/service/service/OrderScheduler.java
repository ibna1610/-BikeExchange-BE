...
public class OrderScheduler {
    ...
    /**
     * Chạy mỗi giờ.
        * Tìm các order ở trạng thái DELIVERED mà deliveredAt đã quá thời gian cấu hình
    ...
    public void autoReleaseExpiredOrders() {
        ...
        LocalDateTime deadline = LocalDateTime.now().minusMinutes(returnWindowTotalMinutes);
        List<Order> expiredOrders = orderService.findExpiredDeliveredOrders(deadline);

        for (Order order : expiredOrders) {
            try {
                orderService.releaseToSeller(order, "Auto-release after return window for Order: " + order.getId());
            } catch (Exception e) {
